...
def check_in_view(request):
    ...
    if request.method == 'POST':
        ...
        # Process check-in - use the same logic as the API
        planned_activities_data = request.POST.get('planned_activities', '').strip()
        daily_goals_data = request.POST.get('daily_goals', '').strip()
        morning_problems = request.POST.get('morning_problems', '').strip()
        
        # Parse activities and goals
        planned_activities_list = [{'title': line.strip()} for line in planned_activities_data.split('\n') if line.strip()]
        daily_goals_list = [{'title': line.strip()} for line in daily_goals_data.split('\n') if line.strip()]
        ...
        messages.success(request, "Check-in successful! Have a productive day!")
        return redirect('activities:daily_summary', activity_id=daily_activity.id)
    
    context = {
        'user': request.user,
        'daily_activity': daily_activity,
        'is_checked_in': daily_activity.checkin_time is not None,
        'has_employee_profile': hasattr(request.user, 'employee_profile'),
    }
    
    if hasattr(request.user, 'employee_profile'):
        context['expected_time'] = request.user.employee_profile.effective_work_start_time
    
    return render(request, 'activities/check_in.html', context)


@login_required
def check_out_view(request):
    """Afternoon check-out view"""
    today = date.today()
    ...
    if request.method == 'POST':
        ...
        for activity in planned_activities:
            activity_status = request.POST.get(f'activity_status_{activity.id}')
            activity_reasons = request.POST.get(f'activity_reasons_{activity.id}', '').strip()
            
            if not activity_status:
                messages.error(request, f'Please set status for activity: {activity.title}')
                return redirect('activities:check_out')
            
            if activity_status in ['not_completed', 'cancelled', 'deferred'] and not activity_reasons:
                messages.error(request, f'Please provide a reason for activity: {activity.title}')
                return redirect('activities:check_out')
        
        # Check daily goals
        for goal in daily_goals:
            goal_status = request.POST.get(f'goal_status_{goal.id}')
            goal_reasons = request.POST.get(f'goal_reasons_{goal.id}', '').strip()
            
            if not goal_status:
                messages.error(request, f'Please set status for goal: {goal.title}')
                return redirect('activities:check_out')
            
            if goal_status in ['not_achieved', 'partially_completed', 'deferred'] and not goal_reasons:
                messages.error(request, f'Please provide a reason for goal: {goal.title}')
                return redirect('activities:check_out')
        
        # Process check-out
        with transaction.atomic():
            daily_activity.checkout_time = timezone.now()
            daily_activity.afternoon_problems = request.POST.get('afternoon_problems', '')
            daily_activity.notes = request.POST.get('notes', '')
            daily_activity.status = 'completed'
            daily_activity.save()
            
            # Update planned activities statuses
            for activity in planned_activities:
                activity_status = request.POST.get(f'activity_status_{activity.id}')
                activity_reasons = request.POST.get(f'activity_reasons_{activity.id}', '').strip()
                
                activity.status = activity_status
                activity.reasons = activity_reasons
                activity.save()
            
            # Update daily goals statuses
            for goal in daily_goals:
                goal_status = request.POST.get(f'goal_status_{goal.id}')
                goal_reasons = request.POST.get(f'goal_reasons_{goal.id}', '').strip()
                goal_completion = request.POST.get(f'goal_completion_{goal.id}', 0)
                goal_achieved_value = request.POST.get(f'goal_achieved_value_{goal.id}', '').strip()
                
                goal.status = goal_status
                goal.reasons = goal_reasons
                goal.completion_percentage = int(goal_completion) if goal_completion else 0
                goal.achieved_value = goal_achieved_value
                goal.save()
            
            # Create additional activities
            additional_activities_text = request.POST.get('additional_activities_text', '').strip()
            if additional_activities_text:
                additional_activities_list = [line.strip() for line in additional_activities_text.split('\n') if line.strip()]
                for i, activity_title in enumerate(additional_activities_list):
                    AdditionalActivity.objects.create(
                        daily_activity=daily_activity,
        ...
        messages.success(request, "Check-out successful! See you tomorrow!")
        return redirect('activities:daily_summary', activity_id=daily_activity.id)
    
    context = {
        'user': request.user,
        'daily_activity': daily_activity,
        'is_checked_out': daily_activity.checkout_time is not None,
        'goals': daily_activity.goals.all(),
        'planned_activities': daily_activity.planned_activities.all(),
        'daily_goals': daily_activity.daily_goals.all(),
        'additional_activities': daily_activity.additional_activities.all(),
        'has_employee_profile': hasattr(request.user, 'employee_profile'),
        'activity_status_choices': PlannedActivity.STATUS_CHOICES,
        'goal_status_choices': DailyGoal.STATUS_CHOICES,
        'additional_activity_categories': AdditionalActivity.CATEGORY_CHOICES,
    }
    
    if hasattr(request.user, 'employee_profile'):
        context['expected_time'] = request.user.employee_profile.effective_work_end_time
    
    return render(request, 'activities/check_out.html', context)
